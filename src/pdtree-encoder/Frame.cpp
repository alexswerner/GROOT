...
bool cmpColor(const Color &p1, const Color &p2) {
    ...
    if (sqrt(p1.r * p1.r + p1.g * p1.g + p1.b * p1.b) <
        sqrt(p2.r * p2.r + p2.g * p2.g + p2.b * p2.b)) {
        return true;
    }
    return false;
}
...
pcl::PointCloud<PointType>::Ptr Frame::getPointCloud() { return cloud_; }

vector<vector<uint8_t>> Frame::getBreadthBytes() { return breadth_bytes_; }

void Frame::transformPointCloud(pcl::PointCloud<PointType>::Ptr cloud,
...
void Frame::generatePayload() {
    ...
    for (int i = 0; i < max_breadth_depth_; i++) {
        payload_.breadth_bytes.insert(payload_.breadth_bytes.end(), breadth_bytes_.at(i).begin(),
                                      breadth_bytes_.at(i).end());
    }
...
void Frame::reorderDepthColorShort() {
    ...
    for (int i = 0; i < numBreadthNodes; i++) {
        ...
        for (int j = startIndex; j < endIndex; j++) {
            ...
            // For debugging
            //            Eigen::Vector4f current_point(cloud_->points[point_index].x,
            //            cloud_->points[point_index].y, cloud_->points[point_index].z, 1.0);
        ...
        sort(reordered_color_nodes.begin() + startIndex, reordered_color_nodes.begin() + endIndex,
             cmpColor);
...
void Frame::writeFrame(std::string filename, unsigned int *avgSize) {
    ...
    int totalSize = sizeof(FrameHeader) + payload_.breadth_bytes.size() +
                    payload_.depth_bytes.size() + payload_.breadth_leaf_indices.size() +
                    payload_.color_bytes.size();
    ...
    *avgSize += totalSize;
    ...
    // printf("Write breadth %d depth %d color %d\n", payload_.breadth_bytes.size(),
    // payload_.depth_bytes.size(), payload_.color_bytes.size());
    ...
    fwrite(&payload_.breadth_leaf_indices[0], sizeof(uint8_t), payload_.breadth_leaf_indices.size(),
           pFile);
    ...
    // printf("Header root center : %f %f %f\n", header_.root_center[0], header_.root_center[1],
    // header_.root_center[2]); printf("Header sidelength: %f\n", header_.root_sidelength);

    cloud_.reset();
}

template <typename T> uint8_t *write_obj(uint8_t *ptr, T const &obj) {
    std::size_t size = sizeof(T);
    std::memcpy(static_cast<void *>(ptr), static_cast<void const *>(&obj), size);
    return ptr + size;
}

uint8_t *write_vec(uint8_t *ptr, std::vector<uint8_t> const &obj) {
    std::size_t size = obj.size();
    std::memcpy(static_cast<void *>(ptr), static_cast<void const *>(obj.data()), size);
    return ptr + size;
}
...
void Frame::getNodesInOctreeContainer(pcl::octree::OctreeNode *currentNode,
                                      std::vector<int> &indices) {
    if (currentNode->getNodeType() == pcl::octree::LEAF_NODE) {
        pcl::octree::OctreeLeafNode<pcl::octree::OctreeContainerPointIndices> *leaf_node =
            static_cast<pcl::octree::OctreeLeafNode<pcl::octree::OctreeContainerPointIndices> *>(
                currentNode);
        ...
        pcl::octree::OctreeBranchNode<pcl::octree::OctreeContainerEmpty> *branch_node =
            static_cast<pcl::octree::OctreeBranchNode<pcl::octree::OctreeContainerEmpty> *>(
                currentNode);
        for (int i = 0; i < 8; i++) {
...
void Frame::readPointCloud(int frame_index, std::string filename, float scale, bool isFlip, float x,
                           float y, float z) {
...
vector<Eigen::Vector4f> Frame::get_debug_point_list() { return debug_point_list_; }

vector<uint8_t> Frame::get_debug_colors() { return debug_color_list_; }
