...
static void *get_depth_bytes(void *t_args) {
    ...
    for (int i = 0; i < thread_args->cnt; i++) {
        int current_val = (*(thread_args->depth_bytes + 3 * i) << 16) +
                          (*(thread_args->depth_bytes + 3 * i + 1) << 8) +
                          *(thread_args->depth_bytes + 3 * i + 2);
        for (int j = 7; j >= 0; j--) {
            int localIdx = (7 - j);
...
void Decoder::decodeBreadthBytes(RxFrameHeader header, uint8_t *breadth_bytes,
